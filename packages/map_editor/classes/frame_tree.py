...
class FrameTree:
    ...
    def add(self, frame_name: str, parent: Optional[str] = None) -> None:
        if parent:
            self.tree.add(successor=frame_name,
                          predecessor=parent)
        else:
    ...
    def remove(self, frame_name: str):
        self.tree.remove(frame_name)

    def predecessor(self, frame_name: str) -> str:
        return list(self.tree.predecessors(frame_name))[0]

    def all_successors(self, frame_name: str) -> List[str]:
        frame_names = self.tree.successors(frame_name)
